...
Macro = [
    '-DSINGLE_PRECISION','-DDOUBLE_PRECISION',
...
def prepareThreadsHeaders(L,include,template,cores):
...
def createLibrary(L,u,include,template,t,m,blas,B,cores): 
    import subprocess
    ...
    prepareThreadsHeaders(L,include,template,cores)
...
def createBreakEven(L,Cores,u,include,template,t,m,blas,B): 
    ...
    print "Thread allocation",L," Unrolling", u, "Starting", B
    prepareThreadsHeaders(L,include,template,len(Cores))
    ...
    while F and attempt<2:
        ...
        if (B>2*originalB) :
            ...
            originalB = B-1
            s = "MACROS=\"%s -DMYSCHED=1 -DLIBRARY_PACKAGE -DAUTOMATIC -DLEAF=%d\" TYPE=%s UNROLL=%d " % (m,originalB,t,u) 
            cmd = '/usr/bin/make clean add-gen %s %s %s' % (gemm, winograd,s)
            print cmd
            process = subprocess.Popen([cmd], shell=True, stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        ...
        cmd = "%s echo \"0 0 1 1 1 %d %d %d %d\" | Executable/%s/%s" % (LIB,B,B,B,B,t,gemmx)
        print cmd
        process = subprocess.Popen([cmd], shell=True, stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        ...
        cmd = "%s echo \"0 0 1 1 1 %d %d %d %d\" | Executable/%s/%s" % (LIB,B,B,B,B,t,winogradx)   
        print cmd
        process = subprocess.Popen([cmd], shell=True, stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
#    print  "createTestAddition", L
    ...
    for l in L:
        ...
        import subprocess
        s = "MACROS=\"%s -DMYSCHED=1 -DLIBRARY_PACKAGE -DAUTOMATIC -DLEAF=%d\" TYPE=%s UNROLL=%d" % (m,NSIZE[Type.index(t)],t,u) 
        ...
        cmd = "%s echo \"0 0 1 1 1 %d %d %d %d\" | Executable/%s/%s" % (LIB,NSIZE[Type.index(t)],NSIZE[Type.index(t)],NSIZE[Type.index(t)],NSIZE[Type.index(t)],t,ex3x)
        print cmd
        process = subprocess.Popen([cmd], shell=True, stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    ...
    while (i< len(Type)):
        ...
        if (len(config)<len(Type)):
            ...
            for u in unrolling:
                ...
                print PER[-1]
                if (len(PER)>=2):
                    error  = PER[-2][0][0]*5/100 
                    error1 = PER[-2][0][1]*5/100 
            ...
            createLibrary(config[i][1],config[i][2],
                          "./PThread/ptarchitecture.h",
                          architecturetemplate,Type[i],Macro[i],blas,config[i][-2],len(Cores))
