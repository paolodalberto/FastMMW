...
main(int argc, char **argv)
{
    ...
    err = clGetDeviceIDs(platforms[platform], CL_DEVICE_TYPE_GPU, MAXDEVICES, device,&ndevices);
    if (err != CL_SUCCESS) {
    ...
    for (i=0; i<Times; i++) {
      ...
      err = clEnqueueWriteBuffer(queue, bufB, CL_FALSE, 0,
				 K * N * sizeof(*B), B, 0, NULL, event +1);
      
      err = clEnqueueWriteBuffer(queue, bufC, CL_FALSE, 0,
				 M * N * sizeof(*C), C, 0, NULL, event +2);
